...
var onCreateGame = function onCreateGame() {
  gamesApi.create().then(gamesUi.onCreateSuccess).catch(gamesUi.onError);
};

var getGames = function getGames() {
  gamesApi.index().then(gamesUi.getGamesSuccess).catch(gamesUi.onError);
};
var getOneGame = function getOneGame(id) {
  gamesApi.show(id).then(gamesUi.getOneGameSuccess).catch(gamesUi.onError);
};

var onUpdateGame = function onUpdateGame(i, value, over) {
  gamesApi.update(i, value, over).then(gamesUi.onUpdateSuccess).catch(gamesUi.onError);
};
...
var playGame = function playGame(element) {
  var player = store.clickCount % 2 === 0 ? 'X' : 'O';
  ...
  if (store.over === false) {
    if (isEmpty(element)) {
      ...
      var isWinner = checkForWinner(store.cells, player);
...
var updateScore = function updateScore(player) {
  var element = '#player_' + player.toLowerCase() + '_score';
...
var checkForWinner = function checkForWinner(cells, player) {
  var val = player;
  if (cells[0] === val && cells[1] === val && cells[2] === val || cells[3] === val && cells[4] === val && cells[5] === val || cells[6] === val && cells[7] === val && cells[8] === val || cells[0] === val && cells[3] === val && cells[6] === val || cells[1] === val && cells[4] === val && cells[7] === val || cells[2] === val && cells[5] === val && cells[8] === val || cells[0] === val && cells[4] === val && cells[8] === val || cells[2] === val && cells[4] === val && cells[6] === val) {
    return true;
  } else {
    return false;
  }
...
var displayLastGame = function displayLastGame() {
  store.cells = store.game.cells;
  var isBlank = checkIfBoardIsBlank(store.cells);
  for (var c = 0; c < store.cells.length; c++) {
    var element = '#' + c;
...
var getNumOfBlankGames = function getNumOfBlankGames() {
  var blank = function blank(x) {
    return x === '';
  };
  var blankGames = store.games.filter(function (obj) {
    return obj.cells.every(blank);
  });
  return blankGames.length;
};

var checkIfBoardIsBlank = function checkIfBoardIsBlank(board) {
  var blank = function blank(x) {
    return x === '';
  };
...
var addGamesPlayed = function addGamesPlayed() {
...
module.exports = {
  ...
  resetAll: resetAll,
  displayLastGame: displayLastGame
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var onSignUp = function onSignUp(event) {
  ...
  var userData = getFormFields(this);
  clearMessage();

  api.signUp(userData).then(function (data) {
    return ui.signUpSuccess(data);
  }).then(function () {
...
var update = function update(i, value, over) {
  return $.ajax({
